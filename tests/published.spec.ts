...
for (const edition of ['main','a','b'] as const) {
  ...
  test(`published ${edition} has the correct readers, accounts, audio and no editor`, async ({ page }) => {
    ...
    await expect(page.locator('.draft-event-venue')).toHaveText('더링크호텔')
    ...
    await expect(page.locator('.draft-event-float')).toHaveCSS('width','94px')
    ...
    await expect(page.locator('.draft-event-venue')).toHaveCSS('font-size','12px')
    await expect(page.locator('.draft-event-hall')).toHaveCSS('font-size','11px')
