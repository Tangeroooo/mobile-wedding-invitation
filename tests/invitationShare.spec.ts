...
test('sharing data preserves each published edition and never shares the editor', () => {
  ...
  for (const variant of ['main', 'a', 'b'] as const) {
    ...
    expect(data.native).toEqual({text:`${data.title}\n${data.text}\n${url}`})
    expect(data.native.text).toBe(data.native.text.trimEnd())
    expect(data.native.text).not.toContain('\n\n')
    expect(data.native.text.split(url)).toHaveLength(2)
